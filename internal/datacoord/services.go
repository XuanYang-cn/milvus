...
func (s *Server) ManualCompaction(ctx context.Context, req *milvuspb.ManualCompactionRequest) (*milvuspb.ManualCompactionResponse, error) {
	log := log.Ctx(ctx).With(
		zap.Int64("collectionID", req.GetCollectionID()),
		zap.Int64("isMajorCompaction", req.GetMajorCompaction()),
		zap.Int64("targetSize", req.GetTargetSize()),
	)
	...
	var id int64
	var err error
	if req.GetMajorCompaction() || req.GetTargetSize() != 0 {
	...
	log.Info("success to trigger manual compaction", zap.Int64("compactionID", id), zap.Int("taskNum", taskCnt))
	return resp, nil
}
...
func (s *Server) GetImportProgress(ctx context.Context, in *internalpb.GetImportProgressRequest) (*internalpb.GetImportProgressResponse, error) {
	...
	progress, state, importedRows, totalRows, reason := GetJobProgress(ctx, jobID, s.importMeta, s.meta, s.jobManager)
	resp.State = state
	resp.Reason = reason
...
func (s *Server) ListImports(ctx context.Context, req *internalpb.ListImportsRequestInternal) (*internalpb.ListImportsResponse, error) {
	...
	for _, job := range jobs {
		progress, state, _, _, reason := GetJobProgress(ctx, job.GetJobID(), s.importMeta, s.meta, s.jobManager)
